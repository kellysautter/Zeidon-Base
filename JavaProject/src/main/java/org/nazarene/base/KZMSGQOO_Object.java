...
public class KZMSGQOO_Object extends VmlObjectOperations
{
   ...
   FindErrorFields( )
   {
      ...
      // TraceLineI( "FindErrorFields SetCursorFirst Task RC: ", nRC );
         ...
         // Load the language object for messages.
         jspLangConv LangConv = new jspLangConv( vMsgQ );  // Language conversion object
         LangConv.getLanguageObject( vMsgQ );
         

         //int nRC = SetCursorFirstEntityByInteger( vMsgQ, "Task", "Id", Integer.valueOf(task.getTask( ).getTaskId( )), "" );
         ...
         // TraceLineI( "FindErrorFields SetCursorFirst WebMsg RC: ", nRC );
            ...
            while ( nRC == 0 )
            {
               ...
               strError = LangConv.getMessageText(GetStringFromAttribute( vMsgQ, "WebMsg", "ErrorMsg" ),GetStringFromAttribute( vMsgQ, "WebMsg", "Id" ));
               if ( s == null || s.isEmpty( ) )
               {
               ...
               s = vMsgQ.cursor( "WebMsg" ).getAttribute( "ErrorMapValue" ).getString();
               sbError.append( "\t\n" );
