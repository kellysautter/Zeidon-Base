...
//******************************************************************************
    ...
    function scwSetLanguage()
        {
			if ( scwLanguage == 'ES' )
            {// Spanish Spanish	enero	feb.	marzo	abr.	mayo	jun.	jul.	agosto	sept./set.	oct.	nov.	dic.
		//	 scwWeekStart = 1;  // This is so that monday is the first day of the week.
             scwToday               = 'Hoy:';
             scwDrag                = 'Haga clic aquí para arrastrar';
             scwArrMonthNames       = ['Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre'];
             scwArrWeekInits        = ['D','L','M','M','J','V','S'];
             scwInvalidDateMsg      = 'La fecha introducida no es válida.\n';
             scwOutOfRangeMsg       = 'La fecha introducida está fuera de rango.';
             scwDoesNotExistMsg     = 'La fecha introducida no existe.';
             scwInvalidAlert        = ['Fecha invalida (',') ignorado.'];
             scwDateDisablingError  = ['Error ',' no es un objeto de fecha.'];
             scwRangeDisablingError = ['Error ',
                                       ' debe consistir en dos elementos.'];
            }
			else
            {// English is default
             scwToday               = 'Today:';
...
// ****************************************************************************
    ...
    // KJS 03/14/16 - I changed scwShow to scwShowWFormat because we were not properly formatting dates that have a
    // context (now we pass context in). scwShow (still two parameters) now calls scwShowWFormat so that older jsp pages will still work.
    function scwShow(scwEle,scwSourceEle)    {scwShowWFormat(scwEle,scwSourceEle,'');}
    function scwShowWFormat(scwEle,scwSourceEle,scwDateFormat)
        {scwTriggerEle = scwSourceEle;
         ...
         // zDateFormat is set on the jsp page. The default is "MM/DD/YYYY".
         // zDateSequence is also on jsp page. The default is "MDY".
         // To not use the default, these are set in the zeidon tools zeidon.ini using
         // WebDateFormat (ex. WebDateFormat=DD.MM.YYYY) and
         // WebDateSequence (ex. WebDateFormat=DMY).
         // The date sequence makes sure we know 09.03.yyyy is either March or September.
         if (scwDateFormat == '')
         {
            scwDateDisplayFormat = document.getElementById("zDateFormat").value;
            scwDateOutputFormat  = scwDateDisplayFormat;
            scwDateInputSequence = document.getElementById("zDateSequence").value; 
            //alert( "Date Format old: " + scwDateDisplayFormat );
         }
         else
         {
            // Passing in the context.
            scwDateDisplayFormat = scwDateFormat;
            scwDateOutputFormat  = scwDateDisplayFormat;
            scwDateInputSequence = document.getElementById("zDateSequence").value; 
            //alert( "Date Format new: " + scwDateDisplayFormat );
         }
         ...
         if (document.getElementById("zLanguage") !==null)
            scwLanguage = document.getElementById("zLanguage").value;
        scwSetDefaultLanguage();
         ...
         document.getElementById('scw').style.zIndex='20';
